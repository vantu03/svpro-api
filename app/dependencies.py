...
def require_session(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)) -> type[UserSession]:
    try:
        ...
        if session_id:
            user_session = db.query(UserSession).filter(UserSession.id == session_id, UserSession.is_active == True).first()
            if user_session:
                return user_session
            else:
                raise HTTPException(
    ...
    raise HTTPException(
        status_code=401,
        detail=response_json(status=False, message="Token không hợp lệ")
    )
